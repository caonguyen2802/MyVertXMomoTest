...
public class JwtGen {
	...
	private static JWTAuth getJWTAuth(Vertx vertx) {
		if (jwtAuth == null) {
			jwtAuth = JWTAuth.create(vertx, new JWTAuthOptions().addPubSecKey(new PubSecKeyOptions().setAlgorithm("RS256")
	...
	private static String genToken(JsonObject data) {
	    return jwtAuth.generateToken(data, new JWTOptions().setAlgorithm("RS256"));
	  }
}
